...
assert(any(audit_mask(:)), 'exact dC/dphi audit support为空。');
...
sample_idx = candidate_idx(order(1:sample_count));
[sample_i, sample_j] = ind2sub(size(lsf), sample_idx);

eps_phi = params.gradient.shadow_fd_eps_factor * params.grid.h;
rows = zeros(sample_count, 7);
fprintf('=== exact dC/dphi finite-difference audit ===\n');
fprintf('eps_phi = %.6e\n', eps_phi);
...
for k = 1:sample_count
    ...
    C_plus = evaluate_candidate_compliance(lsf + eps_phi * e, theta_prev, params.opt.delta_theta_max, ...
        params.grid.dx, params.grid.dy, params.grid.nelx, params.grid.nely, material_mask, ...
        params.material.E_L, params.material.E_T, params.material.nu_LT, params.material.G_LT, ...
        params.material.thickness, params.load.F_mag, params.smooth.eta, params.smooth.iterations);
    C_minus = evaluate_candidate_compliance(lsf - eps_phi * e, theta_prev, params.opt.delta_theta_max, ...
        params.grid.dx, params.grid.dy, params.grid.nelx, params.grid.nely, material_mask, ...
        params.material.E_L, params.material.E_T, params.material.nu_LT, params.material.G_LT, ...
        params.material.thickness, params.load.F_mag, params.smooth.eta, params.smooth.iterations);

    fd = (C_plus - C_minus) / (2 * eps_phi);
    adj = g_exact(i, j);
    rel_err = abs(fd - adj) / max([abs(fd), abs(adj), 1e-12]);
    sign_agree = (abs(fd) < 1e-12 && abs(adj) < 1e-12) || sign(fd) == sign(adj);
    rows(k, :) = [i, j, adj, fd, rel_err, sign_agree, abs(adj)];

    fprintf('sample=%d (i=%d,j=%d): adj=%.6e fd=%.6e rel_err=%.4f sign=%d\n', ...
        k, i, j, adj, fd, rel_err, sign_agree);
end

sign_rate = mean(rows(:, 6));
median_rel_err = median(rows(:, 5), 'omitnan');
fprintf('sign_rate=%.3f\n', sign_rate);
fprintf('median_rel_err=%.6f\n', median_rel_err);

assert(sign_rate >= 0.95, 'exact dC/dphi 与节点有限差分符号一致率过低。');
assert(median_rel_err <= 5e-2, 'exact dC/dphi 与节点有限差分量级不一致。');
fprintf('PASS: exact dC/dphi finite-difference audit.\n');
